...
if [ -f "${VAST_WORKSPACE}/venv/bin/activate" ] && [ -f /comfy_pytorch_pin.txt ]; then
    if ! bash -c "source ${VAST_WORKSPACE}/venv/bin/activate && python -c 'import torchaudio'" 2>/dev/null; then
        TV=$(sed -n '1p' /comfy_pytorch_pin.txt)
        CV=$(sed -n '2p' /comfy_pytorch_pin.txt)
        TVIS=$(sed -n '3p' /comfy_pytorch_pin.txt)
        echo "**** Восстановление torch-стека (ABI) ****"
        source "${VAST_WORKSPACE}/venv/bin/activate" && pip uninstall -y torch torchvision torchaudio 2>/dev/null || true
        if [ "$TVIS" = "legacy" ]; then
...
if [ "${COMFYUI_UPDATE_ON_START,,}" = "true" ] || [ "$COMFYUI_UPDATE_ON_START" = "1" ]; then
    if [ -d "${VAST_WORKSPACE}/ComfyUI/.git" ]; then
        echo "**** COMFYUI_UPDATE_ON_START: обновление ComfyUI из GitHub ****"
        cd "${VAST_WORKSPACE}/ComfyUI" || true
        git config --global --add safe.directory "${VAST_WORKSPACE}/ComfyUI" 2>/dev/null || true
        ...
        if git rev-parse --verify origin/main >/dev/null 2>&1; then
            git checkout -B main origin/main && git reset --hard origin/main && echo "**** ComfyUI → main ****"
        elif git rev-parse --verify origin/master >/dev/null 2>&1; then
            git checkout -B master origin/master && git reset --hard origin/master && echo "**** ComfyUI → master ****"
        else
            echo "**** Нет origin/main/master. Вручную: git fetch --unshallow ****"
        fi
        if [ -f "${VAST_WORKSPACE}/venv/bin/activate" ]; then
            source "${VAST_WORKSPACE}/venv/bin/activate" && pip install --no-cache-dir -q -r "${VAST_WORKSPACE}/ComfyUI/requirements.txt" 2>/dev/null || true
        fi
    else
        echo "**** COMFYUI_UPDATE_ON_START: нет .git — обновление пропущено ****"
    fi
fi
