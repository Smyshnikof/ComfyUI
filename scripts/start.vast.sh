...
start_code_server() {
    ...
    if [[ -n "${ACCESS_PASSWORD}" ]]; then
        ...
        nohup code-server ${VAST_WORKSPACE} --bind-addr 0.0.0.0:8080 \
            --auth password \
            --ignore-last-opened \
            --disable-workspace-trust \
            &> ${VAST_WORKSPACE}/logs/code-server.log &
    else
...
start_comfyui() {
    ...
    nohup python main.py --listen 0.0.0.0 --port 3000 \
        &> ${VAST_WORKSPACE}/logs/comfyui.log &
...
if [ -d /services ]; then
    ...
    nohup uvicorn services.outputs_browser:app --host 0.0.0.0 --port 8083 &> ${VAST_WORKSPACE}/logs/outputs_browser.log &
fi
